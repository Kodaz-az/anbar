...
// Only admin can access this admin-styled page
if (!hasRole('admin')) {
    header('Location: ../auth/unauthorized.php');
    exit;
}

// Get admin's information (used similarly to seller page for unread messages etc.)
...
// Get order details (same queries as seller file, no additional admin-only logic)
...
// Order status text and colors (same as seller version)
$statusConfig = [
...
// Get unread message count for admin (keeps same behavior as seller page but for admin user)
...
<head>
    ...
    <title>Sifariş Detalları | AlumPro Admin</title>
    ...
    <style>
        ...
        /* Admin header/navigation tweaks to match admin/orders.php */
        .app-header .logo { font-weight: 700; }
        .app-header .nav-links a.active { background: rgba(0,0,0,0.03); border-radius: 6px; }

        /* Order details styles (copied from seller file) */
        .order-header {
            display: flex;
        ...
        .status-badge { display: inline-block; padding: 5px 10px; border-radius: 8px; font-size: 13px; font-weight: 500; }
        ...
        .btn { display: inline-flex; align-items: center; gap: 8px; padding: 10px 16px; border-radius: 8px; font-weight: 500; cursor: pointer; transition: all 0.2s; }
        ...
        .detail-cards { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 20px; margin-bottom: 30px; }
        .detail-card { background: white; border-radius: 8px; box-shadow: var(--card-shadow); overflow: hidden; }
        .card-header { background: var(--primary-gradient); color: white; padding: 15px; }
        .card-title { font-size: 16px; font-weight: 500; margin: 0; display:flex; align-items:center; gap:8px; }
        .card-body { padding: 15px; }

        .info-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; }

        .summary-table td { padding: 8px 0; border-bottom: 1px solid #f3f4f6; }
        .summary-label { color: #6b7280; }
        .summary-value { text-align: right; font-weight: 500; color: #374151; }

        .products-table { width: 100%; border-collapse: collapse; margin-bottom: 20px; background: white; border-radius: 8px; overflow: hidden; box-shadow: var(--card-shadow); }
        .products-table th { background: var(--primary-gradient); color: white; padding: 12px 15px; text-align: left; }
        .products-table td { padding: 12px 15px; border-bottom: 1px solid #f0f0f0; }

        .pdf-embed { width: 100%; height: 600px; border-radius: 8px; box-shadow: var(--card-shadow); margin-bottom: 30px; }

        .section-title { font-size: 18px; font-weight: 500; margin: 30px 0 15px 0; padding-bottom: 10px; border-bottom: 1px solid #e5e7eb; display:flex; align-items:center; gap:10px; }

        .barcode-display { background: white; padding: 15px; border-radius: 8px; box-shadow: var(--card-shadow); margin-bottom: 20px; text-align:center; }

        .note-box { background: #f9fafb; border-left: 4px solid var(--primary-color); padding: 15px; border-radius: 0 8px 8px 0; margin-bottom: 20px; }

        .status-dropdown { position: relative; display: inline-block; }
        .status-dropdown-content { display: none; position: absolute; right: 0; background-color: white; min-width: 200px; box-shadow: 0 8px 16px rgba(0, 0, 0, 0.1); z-index: 1; border-radius: 8px; overflow: hidden; }
        .status-dropdown-content a { color: #374151; padding: 12px 16px; text-decoration: none; display:flex; align-items:center; gap:8px; }
        .status-dropdown-content a:hover { background-color: #f3f4f6; }
        .status-dropdown:hover .status-dropdown-content { display: block; }

        .timeline { position: relative; margin-bottom: 30px; padding-left: 30px; }
        .timeline::before { content: ''; position: absolute; top: 0; bottom: 0; left: 15px; width: 2px; background: #e5e7eb; }

        .no-print { display: block; }

        @media print {
            .app-header, .app-footer, .no-print, .order-actions { display: none !important; }
        }
    </style>
</head>
<body>
    <!-- Admin-style Header (from admin/orders.php) -->
        ...
        <div class="app-container">
            <div class="page-header">
                ...
                <div class="breadcrumb">
                    <a href="index.php">Dashboard</a> / <a href="orders.php">Sifarişlər</a> / <span>Detal</span>
                </div>
            </div>
                    ...
                    <div class="detail-card">
                        ...
                        <div class="card-body">
                            <table class="summary-table">
                                ...
                                <?php if (!empty($pricing['transport_fee']) && $pricing['transport_fee'] > 0): ?>
                                    <tr>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (barcodeElement && "<?= addslashes($order['barcode'] ?? '') ?>") {
